...
import { Users, Building2, MessageSquare, TrendingUp, ThumbsUp, ThumbsDown, AlertCircle, Flag } from "lucide-react";

interface AdminStats {
  users: { total: number; admins: number; regular: number };
  businesses: { total: number };
  reviews: { total: number; great: number; concerns: number; generated: number };
}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-600 mt-0.5">Platform health and quick actions</p>
      </div>
      {stats && (
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <CardContent className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="flex items-center gap-2 text-sm text-slate-600">
                  <ThumbsUp className="h-4 w-4 text-green-600" /> Great
                </span>
                <span className="font-semibold text-slate-900">{stats.reviews.great}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="flex items-center gap-2 text-sm text-slate-600">
                  <ThumbsDown className="h-4 w-4 text-red-600" /> Concerns
                </span>
      ...
      <Card className="app-card">
        <CardHeader>
          <CardTitle className="text-lg font-semibold text-slate-900">Support & debugging</CardTitle>
          <CardDescription>Quick access when helping customers or investigating issues</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid sm:grid-cols-2 gap-4">
            <Link href="/admin/system/errors">
              <a className="flex items-center gap-4 p-4 rounded-xl border border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50 transition-colors">
                ...
                <div>
                  <p className="font-medium text-slate-900">Error logs</p>
                  <p className="text-sm text-slate-500">Recent API, webhook, and integration errors</p>
                </div>
              </a>
            </Link>
            <Link href="/admin/system/feature-flags">
              <a className="flex items-center gap-4 p-4 rounded-xl border border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50 transition-colors">
                ...
                <div>
                  <p className="font-medium text-slate-900">Feature flags</p>
                  <p className="text-sm text-slate-500">Enable or disable modules (Google, billing, etc.)</p>
                </div>
              </a>
            </Link>
          </div>
